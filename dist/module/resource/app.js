...
angular.module('we7resource').controller('we7resource-base-controller', [
  ...
  function ($scope, $sce, serviceResource, $http, config) {
    $scope.currentPage = 1;
    //当前页
    $scope.isWechat = config.isWechat;
    // 是否默认显示微信
    $scope.needType = config.needType;
    //config.needType; // 1 返回微信 2 返回本地 3 微信和本地 都行
    $scope.multiple = config.multiple;
    // 是否多选
    $scope.showType = config.showType;
    // 1 微信 2 本地 3 微信和本地都显示
    ...
    if (config.others && config.others[$scope.resourceType]) {
      //如果有特别配置使用特别配置
    ...
    //选中的元素
    $scope.index = 0;
    //3个选项卡 0 微信 1 本地 2 网络
    $scope.converting = false;
    // 是否在转换中
    $scope.showWx = function () {
      return true;  // return $scope.isWechat || $scope.showType == 1 || $scope.showType == 3;
    };
    $scope.showLocal = function () {
      return true;  // return !$scope.isWechat || $scope.showType == 2 || $scope.showType == 3;
    };
    ...
    $scope.delItem = function (item, $event) {
      ...
      serviceResource.delItem(item.id, $scope.resourceType, $scope.index == 1, $scope.uniacid).then(function () {
        $scope.loadData();
      }, function (error) {
    ...
    function doConvert(item) {
      ...
      var totype = $scope.needType == 2 ? true : false;
      // true 转化为本地 false 转化为wechat
      serviceResource.convert(item.id, $scope.resourceType, totype).then(function (item) {
        util.loaded();
        $scope.converting = false;
        if (item) {
          fireSelected([item]);
