...
angular.module('moduleApp').controller('ModuleMoreCtrl', [
  '$scope',
  'config',
...
]).controller('moduleGroupPostCtrl', [
  '$scope',
  '$http',
  'config',
  function ($scope, $http, config) {
    ...
    $scope.selectModule = function (module, type) {
      ...
      if (type == 'module') {
        if ($scope.selectedModules.length >= _.values($scope.groupNotHaveModuleApp).length) {
          //因模块应用 有主模块
  ...
  'config',
  '$sce',
  function ($scope, $http, $compile, config, $sce) {
    $scope.config = config;
    ...
    $scope.change_welcome_module = function (name) {
      ...
      $http.post(config.set_site_welcome_url, { 'name': name }).success(function (data) {
...
]).controller('userModuleDisplayCtrl', [
  '$scope',
  '$http',
  '$timeout',
  'config',
  function ($scope, $http, $timeout, config) {
    ...
    $scope.showAccounts = function (ev, name) {
      require(['underscore'], function () {
        ...
        $http.post(config.links.moduleAccounts, { 'module_name': name }).success(function (data) {
    ...
    $scope.getall_last_switch = function () {
      $http.post(config.links.getall_last_switch).success(function (data) {
