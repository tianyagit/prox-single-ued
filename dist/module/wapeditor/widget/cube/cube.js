angular.module('wapeditorApp').controller('CubeCtrl', [
  '$scope',
  function ($scope) {
    if ($scope.activeItem.params && $scope.activeItem.params.layout && _.isEmpty($scope.activeItem.params.layout)) {
      for (row = 0; row < 4; row++) {
        ...
        for (col = 0; col < 4; col++) {
          $scope.activeItem.params.layout[row][col] = {
            'cols': 1,
            'rows': 1,
            'isempty': true,
            'imgurl': '',
    ...
    $scope.showSelection = function (row, col) {
      $scope.activeItem.params.currentPos = {
        'row': row,
        'col': col
      };
      //构造布局选择数组
      ...
      for (i = row; i < 4; i++) {
        ...
        for (j = col; j < 4; j++) {
          ...
          //console.log(' ====== row:'+i+' - col:'+j);
          $scope.activeItem.params.selection[x][y] = {
            'rows': x,
            'cols': y
          };
          y++;
    ...
    $scope.selectLayout = function (x, y, rowspan, colspan) {
      ...
      $scope.activeItem.params.layout[x][y] = {
        ...
        'isempty': false,
        'imgurl': '',
        'classname': 'index-' + $scope.activeItem.params.showIndex
      };
          ...
          if (!_.isEmpty($scope.activeItem.params.layout[i][j]) && $scope.activeItem.params.currentLayout.classname == $scope.activeItem.params.layout[i][j].classname) {
            if ($scope.activeItem.params.currentLayout.rows > 1) {
              for (var row = 0; row < $scope.activeItem.params.currentLayout.rows; row++) {
                var new_i = i + row;
                $scope.activeItem.params.layout[new_i][j] = {
                  'cols': 1,
                  'rows': 1,
                  'isempty': true,
                  'imgurl': '',
                ...
                if ($scope.activeItem.params.currentLayout.cols > 1) {
                  for (var col = 0; col < $scope.activeItem.params.currentLayout.cols; col++) {
                    var new_j = j + col;
                    $scope.activeItem.params.layout[new_i][new_j] = {
                      'cols': 1,
                      'rows': 1,
                      'isempty': true,
                      'imgurl': '',
              ...
              $scope.activeItem.params.layout[i][j] = {
                'cols': 1,
                'rows': 1,
                'isempty': true,
                'imgurl': '',
              ...
              if ($scope.activeItem.params.currentLayout.cols > 1) {
                for (var col = 0; col < $scope.activeItem.params.currentLayout.cols; col++) {
                  var new_j = j + col;
                  $scope.activeItem.params.layout[i][new_j] = {
                    'cols': 1,
                    'rows': 1,
                    'isempty': true,
                    'imgurl': '',
