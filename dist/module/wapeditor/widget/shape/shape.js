angular.module('wapeditorApp').controller('ShapeCtrl', [
  '$scope',
  '$http',
  function ($scope, $http) {
    $scope.page = {
    ...
    $scope.addItem = function () {
      $http({
        ...
        $scope.activeItem.params.catlistActive = 1;
        //默认active大分类ID
        $scope.activeItem.params.imgListActive = 4;
        //默认active小分类ID,控制小分类正确显示
        ...
        if (start > 0) {
          if (end <= $scope.page.numPages) {
            pagesList = [
              ...
              {
                'number': $scope.page.currentPage - 1,
                'active': false
              },
              {
                'number': $scope.page.currentPage,
                'active': true
              },
              {
                'number': $scope.page.currentPage + 1,
    ...
    var getTotalItems = function (parentid) {
      ...
      for (var i in $scope.sysImageList) {
        if (parentid == $scope.sysImageList[i].parentid)
          sum++;
      }
      return sum;
    };
