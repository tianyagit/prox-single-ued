angular.module('wapeditorApp').service('serviceSetStyle', [
  '$rootScope',
  '$timeout',
  'serviceCommon',
  'config',
  function ($rootScope, $timeout, serviceCommon, config) {
    var serviceSetStyle = {};
    var i = 1;
    //initStyleParams()函数初始化位置时使用
    ...
    serviceSetStyle.cssCompatible = function (type, value) {
      ...
      if (angular.isString(type) && angular.isString(value)) {
        newTypeStyle = type + ': ' + value + ';-webkit-' + type + ': ' + value + ';-moz-' + type + ': ' + value + ';-o-' + type + ': ' + value + ';-ms-' + type + ': ' + value + ';';
        return newTypeStyle;
      }
    };
