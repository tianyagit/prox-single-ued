...
angular.module('webApp').controller('webappModuleLinkUniacidCtrl', [
  '$scope',
  '$http',
  'config',
  function ($scope, $http, config) {
    ...
    $scope.searchLinkAccount = function (modulename, type) {
      ...
      $http.post(config.links.search_link_account, {
        ...
        'type': type == 'wxapp' ? config.wxapp : config.app
...
angular.module('webApp').controller('bindDomainCtrl', [
  '$scope',
  '$http',
  'config',
  function ($scope, $http, config) {
    ...
    $scope.httpChange = function () {
      $http.post(config.links.postDomain, {
    ...
    $scope.changeModule = function () {
      $http.post(config.links.postModule, {
        'module_name': $scope.selectedModule,
        'token': config.token
      }).success(function (data) {
        if (data.message.errno == 0) {
          util.message('\u4fee\u6539\u6210\u529f\uff01', data.redirect, 'success');
