angular.module('xzApp').service('XzAppCommon', [
  '$rootScope',
  '$http',
  'config',
  function ($rootScope, $http, config) {
    var XzAppCommon = {};
    XzAppCommon.addPermission = function () {
    ...
    XzAppCommon.update_package_list = function (package) {
    ...
    XzAppCommon.selectOwner = function () {
      var seletedUserIds = [];
      require(['biz'], function (biz) {
        biz.user.browser(seletedUserIds, function (user) {
          $http.post(config.links.userinfo, { uid: user }).success(function (data) {
            ...
            $('.account-package-extra').show();
            XzAppCommon.update_package_list(data.message.message.package);
          });
        }, {
          mode: 'invisible',
          direct: true
    ...
    XzAppCommon.copySuccess = function (id, obj) {
      var id = parseInt(id);
      var obj = obj;
    ...
    XzAppCommon.encodingAESKeyGen = function () {
