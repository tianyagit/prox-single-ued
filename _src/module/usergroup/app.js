...
//用户组管理-添加/编辑
angular.module('userGroup').controller('UserGroupPost', ['$scope', '$compile', 'config', function($scope, $compile, config){
...
//副创始人组管理列表
angular.module('userGroup').controller('ViceGroupDisplay', ['$scope', 'config', function($scope, config){
	$scope.lists = config.lists;
	$scope.links = config.links;
	$scope.editGroup = function(id) {
		var id = parseInt(id);
...
//副创始人管理-添加/编辑
angular.module('userGroup').controller('ViceGroupPost', ['$scope', 'config', function($scope, config){
	$scope.groupInfo = config.groupInfo;
	$scope.packages = config.packages;
	$scope.changeText = function(ev) {
		var text = $(ev)[0].target.text;
