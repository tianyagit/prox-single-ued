...
angular.module('we7codeAppjsonApp').controller('code_appjson_ctrl',
	['$scope', '$q', 'config', '$http', function ($scope, $q, config, $http) {
		...
		$('body').on('click', '.js-image', function () {
			...
			util.image({}, function (item) {
				$http.post(convert_img_url, { 'version_id': config.version_id, 'att_id': item.id }).then(function (data) {
					if (data.data.message.errno == 0) {
						$scope.tabBar.list[index][pathname] = data.data.message.message;
						//						$scope.$digest();
					}
		...
		$scope.save = function (item) {
			...
			$http.post(save_url, { json: json, 'version_id': config.version_id }).then(function (data) {
				if (data.data.message.errno == 0 ) {
		...
		util.colorpicker('.js-color', function () { });//回调不做任何处理
	}]);