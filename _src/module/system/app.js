...
//系统菜单管理
angular.module('systemApp').controller('MenuCtrl', function($scope, $http) {
	$scope.subItemGroup = {};
	$scope.displayStatus = {};
	$scope.mainMenu = {'permission' : '', 'displayorder' : 0};
	...
	$scope.changeDisplay = function(key) {
		...
		$http.post('./index.php?c=system&a=menu&do=display_status', {'status' : status, 'permission_name' : key}).success(function (data) {
			$scope.displayStatus[key] = parseInt(status) ? true : false;
		});
	};
});
//系统管理欢迎
        angular.module('systemApp').controller('WelcomeCtrl', ['$scope', '$http', 'config', function($scope, $http, config){
            //微擎推荐模块
            $scope.loaderror = 0;
            $scope.ads = null;
            //模块统计信息
...
angular.module('systemApp').controller('sensitiveWord', ['$scope', '$http', 'config', function($scope, $http, config){
	$scope.lists = config.lists;
	$scope.links = config.links;
	$scope.word = '';
...
angular.module('systemApp').controller('SystemThirdpartyLogin', ['$scope', '$http','serviceCommon', 'config', function($scope, $http,serviceCommon, config){
    ...
    $scope.httpChange = function(set_type, auth_type) {
        switch(set_type) {
            case 'authstate':
                $http.post($scope.links.save_setting, {'authstate':'authstate', 'type': auth_type})
                    .success(function(data){
                        if(data.message.errno == 0) {
            ...
            case 'appid':
                $('#AppID').modal('hide');
                $http.post($scope.links.save_setting, {'appid': $scope.newappid, 'type': auth_type})
                    .success(function(data){
                        if(data.message.errno == 0) {
                            util.message('修改成功', data.redirect);
                        }else {
            ...
            case 'appsecret':
                ...
                $http.post($scope.links.save_setting, {'appsecret': $scope.newappsecret, 'type': auth_type})
                    .success(function(data){
                        if(data.message.errno == 0) {
                            util.message('修改成功', data.redirect);
                        }else {
...
angular.module('systemApp').controller('systemOauthCtrl', ['$scope', '$http', 'config', function($scope, $http, config){
	...
	$scope.saveOauth = function() {
		$http.post($scope.links.oauth_link, {'account' : $scope.oauthAccount, 'host' : $scope.oauthHost}).success(function(data){
			if (data.message.errno != 0) {
				util.message(data.message.message);
			} else {
...
//设置系统短信签名
angular.module('systemApp').controller('SmsSignCtrl', ['$scope','$http', 'config', function($scope, $http, config){
...
angular.module('systemApp').controller('systemInfoCtrl', ['$scope','$http', 'config', function($scope, $http, config){
    $scope.content = '查看';
