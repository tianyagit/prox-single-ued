...
angular.module('we7resource').directive('we7ResourceModuleDialog', ['$http', function ($http) {
	return {
		scope: {},
		restrict: 'EA',
...
angular.module('we7resource').controller('we7resource-module-controller',
	['$scope', '$sce', 'serviceResource', 'config', function ($scope, $sce, serviceResource, config) {
		...
		$scope.itemClick = function (item) {
			if ($scope.multiple) {
				...
				item.selected = true;
				$(window).trigger('resource_selected', { type: 'module', items: [item] });
			}
		...
		function fireSelected(items) {
			$(window).trigger('resource_selected', { type: 'module', items: items });
		}
		...
		function loadData() {
			//模块不管是不是本地
			serviceResource.getResources('module', $scope.currentPage, true, { keyword: $scope.keyword, user_module: user_module, mtype: mtype , cover: cover}).then(function (data) {
