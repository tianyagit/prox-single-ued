...
angular.module('we7resource').controller('we7resource-image-controller',
	['$scope', '$sce', 'serviceResource', '$http', '$controller', 'config',
		function ($scope, $sce, serviceResource, $http, $controller, config) {
			...
			$scope.multipleupload = true; //多文件上传
			$scope.quality = 0;
			$scope.netWorkurl = '';
			$scope.groups = [];

			var now = new Date();
			$scope.year = '0'; //now.getFullYear();
			$scope.month = '0'; //now.getMonth()+1;
			...
			function fireSelected(items) {
				$(window).trigger('resource_selected', { type: 'image', items: items });
			};
  ...
  */
			...
			function setUploadUrl() {
				...
				$scope.uploadurl = $scope.index == 0 ? './index.php?c=utility&a=file&do=wechat_upload&upload_type=image&mode=perm&uniacid=' + $scope.uniacid + '&dest_dir=' + $scope.dest_dir + '&quality=' + $scope.quality + '&group_id='+groupid:
					'./index.php?c=utility&a=file&do=upload&upload_type=image&global=' + $scope.global + '&dest_dir=' + $scope.dest_dir + '&uniacid=' + $scope.uniacid + '&quality=' + $scope.quality + '&group_id='+groupid;
			}
			...
			$scope.addGroup = function (name) {
				...
				serviceResource.addGroup(groupname, isLocal).then(function (data) {
					$scope.groups.push({ name: groupname, id: data.id, editable: false, deleted: false, changed: false });
				});
			};
			...
			function netWorkconvert(url, toLocal) {
				...
				serviceResource.netWorkconvert(url, toLocal, 'image')
					.then(function (item) {
						util.loaded();
						fireSelected( [item]);
					}, function (error) {
			...
			function loadImages(groupid) {
				...
				setUploadUrl();// 上传路径添加分组id
				var isLocal = $scope.index == 1;
				serviceResource.getResources('image', $scope.currentPage, isLocal, { year: $scope.year, month: $scope.month, uniacid: $scope.uniacid, dest_dir: $scope.dest_dir, global: $scope.global, groupid: groupid }).then(function (data) {
