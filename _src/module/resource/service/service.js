...
angular.module('we7resource').service('serviceResource',
	...
	**/
		serviceResource.getResources = function (key, pageNo, islocal, queryParams) {
			...
			switch (key) {
			case 'keyword': promiseResult = getKeyWordResource(pageNo, queryParams); break;
			case 'module': promiseResult = getModuleResource(pageNo, queryParams); break;
			case 'video': promiseResult = getMaterialResource('video', pageNo, islocal); break;
			case 'news': promiseResult = getMaterialResource('news', pageNo, islocal, queryParams); break;
			case 'voice': promiseResult = getMaterialResource('voice', pageNo, islocal, queryParams); break;
			case 'image': promiseResult = getMaterialResource('image', pageNo, islocal, queryParams); break;
		...
		serviceResource.imageGroup = function(islocal, queryparams) {
			...
			$http.get(url).then(function(response){
				if (response.status == 200) {
					...
					deferred.reject({ state: false, message: data.message });
				}
			});
			return promise;
		...
		serviceResource.addGroup = function(groupname, islocal, queryparams)  {
			...
			$http.post(url, { 'name': groupname}).then(
				function (response) {
					if (response.status == 200) {
						var data = response.data.message;
						if (data.errno == '0') {
					...
					deferred.reject({ state: false, message: '添加失败' });
				}, function (error) {
					deferred.reject({ state: false, message: '添加失败' });
				});
			return promise;
		...
		serviceResource.changeGroup = function (group, islocal, queryparams) {
			...
			$http.post(url, { 'name': group.name, 'id': group.id }).then(
				function (response) {
					if (response.status == 200) {
						var data = response.data.message;
						if (data.errno == '0') {
					...
					deferred.reject({ state: false, message: '更新失败' });
				}, function (error) {
					deferred.reject({ state: false, message: '更新失败' });
				});
			return promise;
		...
		serviceResource.delGroup = function (groupid, islocal, queryparams) {
			...
			$http.post(url, { 'id': groupid }).then(
				function (response) {
					...
					deferred.reject({ state: false, message: '删除失败' });
				}, function (error) {
					deferred.reject({ state: false, message: '删除失败' });
				});
			return promise;
		...
		serviceResource.moveToGroup = function (keys, groupid, islocal, queryparams) {
			...
			$http.post(url, { 'id': groupid, keys:keys }).then(
				function (response) {
					if (response.status == 200) {
						var data = response.data.message;
						if (data.errno == '0') { 
					...
					deferred.reject({ state: false, message: '移动成功' });
				}, function (error) {
					deferred.reject({ state: false, message: '移动失败' });
				});
			return promise;
		}
		...
		serviceResource.delMuti = function (keys, type, islocal, queryParams) {
			...
			$http.post(url, { 'id': keys, 'type': type }).then(
				function (response) {
					if (response.status == 200) {
						var data = response.data.message;
						if (data.errno == '0') {
				...
				function (response) {
					if (response.status == 200) {
						...
						if (data.errno == '0') {
							...
							deferred.resolve({ pager: data.message.pager, items: items });
						}
					}
					deferred.resolve([]);
				}, function (error) {
		...
		serviceResource.delItem = function (resourceId, type, islocal, uniacid) {
			...
			$http.post(url, { 'material_id': resourceId, 'type': type, 'server': server }).then(
				function (response) {
					if (response.status == 200) {
						var data = response.data.message;
						if (data.errno == '0') {
		...
		serviceResource.convert = function (resourceId, type, toLocal) {
			...
			var url = './index.php?c=utility&a=file&do=' + convertType +
				'&type=' + type + '&resource_id=' + resourceId;
			$http.get(url).then(
				function (response) {
					if (response.status == 200) {
						var data = response.data.message;
		...
		serviceResource.netWorkconvert = function (mediaUrl, toLocal, type) {
			...
			var url = './index.php?c=utility&a=file&do=' + convertType +
				'&url=' + encodeURIComponent(mediaUrl) + '&type=' + type;
			$http.get(url).then(
				function (response) {
					if (response.status == 200) {
						var data = response.data.message;
