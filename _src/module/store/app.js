...
angular.module('storeApp').controller('goodsSellerCtrl', ['$scope', '$http', 'config', function($scope, $http, config) {
	...
	$scope.editPrice = function(type) {
		switch(type) {
			case 'add_module':
				$http.post(config.links.add, {'module' : $scope.selectedModule, 'toedit': true})
				...
				break;
			case 'add_api':
				$http.post(config.links.add, {'title': '应用访问流量(API)', 'visit_times': $scope.visitTimes, 'price': $scope.visitPrice, 'online': true})
					.success(function(data) {
						if (data.message.errno == 0) {
	...
	$scope.toOffline = function(type) {
		switch(type) {
			case 'add_module':
				$http.post(config.links.add, {'module' : $scope.selectedModule})
					.success(function(data) {
						if (data.message.errno == 0) {
				...
				break;
			case 'add_api':
				$http.post(config.links.add, {'title': '应用访问流量(API)', 'visit_times': $scope.visitTimes, 'price': $scope.visitPrice})
					.success(function(data) {
						if (data.message.errno == 0) {
							location.href = config.links.offline;
						} else {
...
angular.module('storeApp').controller('goodsPostCtrl', ['$scope', '$http', '$compile', 'config', function($scope, $http, $compile, config) {
    $scope.slideLists = [];
    ...
    $scope.addGroupPrice = function (group_id, group_name, price) {
        ...
        var html = '<div class="form-group form-group-'+ index +'">' +
            '<label class="control-label col-sm-2"></label>' +
            '<div class="col-sm-' + w + '">' +
            '<div class="input-group">' +
            '<div class="input-group-btn">' +
            '<button type="button" class="btn btn-info dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="min-width:120px;background-color: #f8f9f9;color: #666;">' +
            '<span class="group-title">' + (price ? group_name : '选择用户组') + '</span> ' +
            '<span class="caret" style="color: #3071a9"></span>' +
            '<input type="hidden" name="user_group_id[]" value="' + group_id + '"/>' +
            '<input type="hidden" name="user_group_name[]" value="' + group_name + '"/>' +
            '</button>' +
            '<ul class="dropdown-menu dropdown-menu-right">';
        html += liHmlt;
        html += '</ul>' +
            '</div>' +
            '<input type="number" class="form-control" min="0" name="user_group_price[]" value="' + price + '" step="0.01">' +
            '<span class="input-group-addon" >元 / <span ng-if="num > 0" ng-bind="num"></span><span ng-bind="unitTitle"></span></span>' +
            '<div class="input-group-btn" style="padding-left: 10px"><button type="button" class="btn btn-default" ng-click="deleteGroupPrice('+ index +')">删除</button></div>' +
            '</div>' +
            '</div>' +
            '</div>';
    ...
    $scope.getuserGroupsLi = function (index) {
        ...
        for (key in $scope.userGroups) {
            html += '<li>' +
                '<a href="" ng-click="changeGroup('+ index +', $event)" ' +
                'data-title="' + $scope.userGroups[key].name +
                '" data-id="' + $scope.userGroups[key].id +
                '">'
                + $scope.userGroups[key].name +
                '</a>' +
                '</li>';
        }
        return html;
    ...
    $scope.addSlide = function() {
        require(['fileUploader'], function(uploader){
            ...
            }, {'direct' : true, 'multiple' : false, 'uniacid' : -1});
        });
    };
...
angular.module('storeApp').controller('storePaySettingCtrl', ['$scope', '$http', 'config', function($scope, $http, config) {
	$scope.alipay = config.alipay;
}]);
angular.module('storeApp').controller('storeOrdersCtrl', ['$scope', '$http', 'config', function($scope, $http, config){
	...
	$scope.changePrice = function() {
		$http.post(config.links.changePrice, {'id': $scope.newPrice.orderid, 'price': $scope.newPrice.price})
			.success(function(data) {
				if (data.message.errno == 0) {
...
angular.module('storeApp').controller('goodsBuyerCtrl', ['$scope', '$http', 'config', function($scope, $http, config) {
	$scope.duration = 1;
	...
	$scope.changeExpire = function(duration, uniacid) {
		...
		$http.post('./index.php?c=site&a=entry&operate=change_order_expire&direct=1&do=changeorderexpire&m=store', {'duration' : duration, 'unit' : $scope.unit, 'uniacid' : uniacid, 'goodsid' : $scope.goods.id}).success(function(data) {
			if (data.message.errno == 0) {
...
angular.module('storeApp').controller('storePermissionCtrl', ['$scope', '$http', 'config', function($scope, $http, config) {
	...
	$scope.changeStatus = function() {
		$http.post(config.links.changeStatus, {'type': $scope.type})
			.success(function(data) {
				console.log(data);
				if (data.message.errno == 0) {
					if ($scope.type == 'close') {
